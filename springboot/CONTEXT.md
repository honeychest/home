...
- 자원(CPU/RAM/DISK) 임계는 `StatusLadder.RESOURCE_PCT`(70 경고 / 80 다운)가 단일 소스 —
  보드 표시·판정과 AlertService 의 CRITICAL 알림 임계가 같은 상수의 `downAt()` 을 읽는다.
  과거 AlertService 가 리터럴 `80d` 을 따로 들고 주석으로만 "동일값"을 약속해 한쪽만 바꾸면
  조용히 어긋날 수 있었던 이중화를 제거(AlertService 가 상수를 읽는 방향). (2026-07)
