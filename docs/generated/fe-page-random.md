...
**한 줄 정의** — Matter.js 물리 엔진으로 공을 굴려 무작위로 당첨자를 뽑는 추첨(Random Picker) 페이지. (경로 `/winner`)

**누가·언제 쓰나** — 참가자 중 공정하게 무작위로 당첨자를 뽑고 싶을 때.

**핵심 기능 3가지**
1. 물리 시뮬레이션 보드(핀·디플렉터·골 센서)
2. 승리 판정·랭킹 시스템
3. 레이아웃 편집기(`/winner/editor`)
