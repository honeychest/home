...
## 역할 요약
- **한 줄 정의** — 시스템 25개 크리티컬 체크(하트비트 6개)의 현재 상태를 한 화면에 모아 "전부 정상(OK)"을 확인하고, 이상 시 원인·최근 실패 이력을 즉시 보는 운영자 전용 독립 페이지.
...
- **작업 인수인계(HandoffPanel, 기본 펼침)**: 계측 진행(25/25 완료 — 피드 4·하트비트 6·리소스 4·인프라 3·데이터 2·외부 6), 5종 계측 패턴, 핵심 파일, 새 체크 추가 3단계, 아키텍처 개선 인계. **개발 현황의 단일 소스**.
- **운영 체크리스트·장기 메모(NotesPanel, 기본 접힘)**: 임계값 실측 튜닝 TODO, 운영 주의점(텔레그램 전제·DOWN만 알림·leader 기준 값·리테이션 등), 배포 전 체크리스트.

## 연관 도메인
- 백엔드: `be-health`(이 화면이 소비하는 `/api/admin/health/**` API의 전부 — 25개 체크·판정·이력·알림). 권한·레이아웃은 `fe-shared`(`useAdminAuth`·`Layout`·`adminAccessPolicy`), HTTP는 `fe-api`(`apiClient`).
